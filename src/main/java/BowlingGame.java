public class BowlingGame {
    ...
    public int getFrameFinalScore(int[][] scorecard, int frame) {
        if (isSpareOrStrikeInTheTenthFrame(scorecard, frame)) {
    ...
    private boolean isSpareOrStrikeInTheTenthFrame(int[][] scorecard, int frame) {
        return frame == 10 && (isSpare(scorecard[frame - 1]) || isStrike(scorecard[frame - 1]));
    }
